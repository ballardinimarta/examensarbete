...
function InstagramFeed() {
    ...
    return (
        ...
        <div className={styles.instagramFeed}>
            {user?.data.map((post)=> {
                return (
                    post.media_type=== "VIDEO" ? null :
                        <div className={styles.instaImg} key={post.id}>
                            <img src={post.media_url} alt={post.caption}/>
                            <div className={styles.overlay}>
                                <span className={styles.overlayText}><a href='https://www.instagram.com/doktor.drej/' className={styles.overlayTextAt}  target="_blank" rel="noreferrer">@Doktor.Drej</a> {post.caption}</span>
                            </div>
                        </div>
            )
            })} 
        </div>
        <button className='blackButton' onClick={showMore}>Visa mer!</button>
        </>
    )
}
