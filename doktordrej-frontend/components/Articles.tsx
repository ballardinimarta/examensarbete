...
function Articles() {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      setCategory("Alla");
      setEndpoint(
        `stories?filter_query[name][like]=*${searchTerm}*&per_page=${pageSize}&page=1&starts_with=articles&cv=${timestamp}&token=${process.env.token}&sort_by=content.isSold:asc`
      );
      setSearchLoad(false);
    }, 1500);
